...
if (!NEXT_PUBLIC_BSC_RPC_URL)
  throw new Error("Missing environment variable: NEXT_PUBLIC_BSC_RPC_URL");
if (!BEP20_ADDRESS)
  throw new Error(
    "Missing environment variable: NEXT_PUBLIC_BEP20_CONTRACT_ADDRESS"
  );
if (!NEXT_PUBLIC_TRON_RPC_URL)
  throw new Error("Missing environment variable: NEXT_PUBLIC_TRON_RPC_URL");
if (!TRC20_ADDRESS_BASE58)
  throw new Error(
    "Missing environment variable: NEXT_PUBLIC_TRC20_CONTRACT_ADDRESS"
  );
...
try {
  ...
  throw new Error(
    `Invalid NEXT_PUBLIC_TRC20_CONTRACT_ADDRESS format: ${TRC20_ADDRESS_BASE58}`
  );
}
// --- End Configuration ---
...
export function getTronLinkProvider(): TronLinkProvider | null {
  ...
  if (!tronLink.ready) {
    ...
    console.warn(
      "TronLink is installed but not ready. User might need to log in or setup."
    );
  ...
  if (!tronLink.tronWeb) {
    console.warn(
      "TronLink found and ready, but 'tronWeb' instance is not yet injected. May need a short delay or re-check."
    );
 ...
 */
export async function getTrc20Contract(
  tronWebInstance: TronWebInstance
): Promise<any> {
  // tronWeb.contract returns 'any'
  if (!tronWebInstance?.contract) {
    throw new Error(
      "TronWeb instance or its 'contract' method is not available."
    );
  }
  try {
    // TronWeb might cache instances based on address
    const contract = await tronWebInstance.contract(
      TRC20_ABI.abi,
      TRC20_ADDRESS_HEX
    );
    if (!contract?.methods) {
      throw new Error(
        `Failed to initialize TRC20 contract instance at ${TRC20_ADDRESS_BASE58}. Check ABI and address.`
      );
    }
    return contract;
  } catch (error) {
...
// Initialize provider conditionally to avoid errors when window.ethereum is null
const provider =
...
export async function mintBep20Tokens(recipient: any, amount: number) {
  try {
    if (!provider) {
      throw new Error(
        "MetaMask or compatible provider not detected. Please install MetaMask and refresh the page."
      );
    }
 ...
 */
export async function mintTrc20Tokens(
  tronWebInstance: TronWebInstance,
  recipient: string,
  amount: string
): Promise<string> {
  if (!tronWebInstance?.defaultAddress?.base58) {
    throw new Error(
      "TronLink wallet instance with address is required for minting TRC-20 tokens."
    );
  }
  try {
    ...
    const rawAmount = BigInt(
      Math.round(parseFloat(amount) * 10 ** tokenDecimals)
    ).toString();

    console.log(
      `Minting TRC20: To=${recipient}, RawAmount=${rawAmount} (Decimals: ${tokenDecimals})`
    );

    // Ensure the method exists on the contract object
    if (!contract.methods.mint) {
      throw new Error(
        "Contract ABI doesn't seem to have a 'mint(address,uint256)' function."
      );
    }
    ...
    if (!txID || typeof txID !== "string" || txID.length < 60) {
      // Basic sanity check for Tron TxID
      throw new Error(
        "Invalid or missing transaction ID received from TronLink."
      );
    }
    return txID;
  } catch (error: any) {
 ...
 */
export async function burnBep20Tokens(
  sign: Signer,
  ...
  if (!sign)
    throw new Error(
      "A connected Signer (wallet) is required for burning BEP-20 tokens."
    );
  if (!provider) {
    throw new Error(
      "MetaMask or compatible provider not detected. Please install MetaMask and refresh the page."
  ...
  try {
    ...
    let message =
      error.shortMessage || error.message || "BEP-20 burning failed";
    if (error.message.includes("execution reverted")) {
 ...
 */
export async function burnTrc20Tokens(
  tronWebInstance: TronWebInstance,
  targetAddress: string,
  amount: string
): Promise<string> {
  if (!tronWebInstance?.defaultAddress?.base58) {
    throw new Error(
      "TronLink wallet instance with address is required for burning TRC-20 tokens."
    );
  }
  try {
    const contract = await getTrc20Contract(tronWebInstance);
    ...
    console.log(
      `Burning TRC20: Target=${targetAddress}, RawAmount=${rawAmount} (Decimals: ${tokenDecimals})`
    );

    // --- IMPORTANT: Adjust method name 'burn' if your contract uses 'burnFrom' or similar ---
    if (!contract.methods.burnFrom) {
      // Adjust this line based on your contract's burn function name
      throw new Error(
        "Contract ABI doesn't seem to have a 'burn(address,uint256)' function. Check ABI/Contract."
      );
    }
    // ---

    const txID = await contract.methods
      .burnFrom(targetAddress, rawAmount)
      .send({
...
export async function getBep20Balance(
  ...
  // Check if it's a Signer with a provider
    ...
    provider.provider &&
    typeof provider.provider.getNetwork === "function"
  ) {
...
export async function getTrc20Balance(
  ...
  try {
    ...
    console.error(
      `Error fetching TRC20 balance for ${address}:`,
      error.message
    );
    return "Error";
  }
...
export function checkTronNetwork(
  tronWebInstance: TronWebInstance | null
): boolean {
  ...
  console.warn(
    `Unrecognized ACTIVE_TRON_CHAIN_ID: ${ACTIVE_TRON_CHAIN_ID}. Cannot verify Tron network.`
  );
...
export async function requestNetworkSwitch(
  ...
  if (!provider?.send) {
    throw new Error(
      "Wallet provider is not available or does not support 'send' method for switching network."
    );
  }
    ...
    if (switchError.code === 4902) {
      ...
      try {
        ...
        /* else if (chainId === 97) { // BSC Testnet
          ...
          throw new Error(
            `Configuration to add chainId ${chainId} is missing.`
          );
        }
      } catch (addError: any) {
      ...
      throw new Error(
        `Failed to switch network: ${
          switchError.message || "User rejected or unknown error"
        }`
      );
    }
  }
