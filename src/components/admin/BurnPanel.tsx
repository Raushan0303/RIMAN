...
export default function BurnPanel({
  ...
  const BurnSchema = z.object({
    targetWallet: z
      .string()
      .trim()
      .refine((addr) => isValidAddress(addr, selectedChain), {
    ...
    amount: z
      ...
      .refine(
        (val) =>
          targetBalance === null ||
          parseFloat(val) <= parseFloat(targetBalance),
  ...
  useEffect(() => {
    const checkBalance = async () => {
      if (
        watchedTargetWallet &&
        isValidAddress(watchedTargetWallet, selectedChain) &&
        connectedWallet &&
        walletProvider &&
        !isWrongNetwork
      ) {
        ...
        try {
          ...
          if (selectedChain === "BSC") {
            balance = await getBep20Balance(
              walletProvider,
              watchedTargetWallet
            );
          } else {
            balance = await getTrc20Balance(
              walletProvider,
              watchedTargetWallet
            );
          }
  ...
  const handleBurnSubmit = async (data: BurnFormInputs) => {
    if (!connectedWallet || !walletProvider || isWrongNetwork) {
      toast({
        variant: "destructive",
        title: "Wallet Issue",
        description: "Please connect the correct wallet and network.",
      });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          apiResult.error || "Failed to record burn transaction."
        );
      }
  ...
  const isSubmitDisabled =
    !isValid || isBurning || !connectedWallet || isWrongNetwork;
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-md p-3 text-sm">
              ...
              {isWrongNetwork && (
                <p className="text-red-400">
                  Please switch to the {selectedChain} network
                </p>
              )}
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            {!fetchingBalance && targetBalance !== null && (
              <p className="text-xs text-gray-400 mt-1">
                Available: {targetBalance} tokens
              </p>
            )}
            {errors.targetWallet && (
              <p className="text-sm text-red-400 mt-1">
                {errors.targetWallet.message}
              </p>
            )}
          </div>
            ...
            {isBurning ? (
              ...
              <div className="flex items-center justify-center">
                <Flame
                  className={cn(
                    "h-4 w-4 mr-2",
                    confirmingBurn && "text-red-400 animate-pulse"
                  )}
                />
